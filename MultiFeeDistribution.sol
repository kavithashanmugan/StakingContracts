...
contract MultiFeeDistribution is ReentrancyGuard, Ownable {
    ...
    // Withdraw all locked assets before unlocking period Finishes
    function withdrawLockedAssets() external {
        LockedBalance[] storage locks = userLocks[msg.sender];
        Balances storage bal = balances[msg.sender];
        require(bal.locked > 0, "Insufficient locked balance");
        uint256 lockedPenaltyAmount;
        uint256 amountWithoutPenalty;
        uint256 lockedAmount;
        uint256 length = locks.length;
        if (locks[length-1].unlockTime <= block.timestamp) {
            amountWithoutPenalty = bal.locked;
            delete userLocks[msg.sender];
        } else {
            for (uint i = 0; i < length; i++) {
        ...
        lockedPenaltyAmount = lockedPenaltyAmount.add(lockedAmount).div(2);
        uint256 totalAmount = amountWithoutPenalty.add(lockedAmount.sub(lockedPenaltyAmount));
        bal.locked = 0;
        bal.total = bal.total.sub(totalAmount);
        totalSupply = totalSupply.sub(totalAmount);
        lockedSupply = lockedSupply.sub(totalAmount);
        stakingToken.safeTransfer(msg.sender, totalAmount);
         if (lockedPenaltyAmount > 0) {
            _notifyReward(address(stakingToken), lockedPenaltyAmount);
        }
